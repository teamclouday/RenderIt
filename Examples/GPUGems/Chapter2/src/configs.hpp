...
struct SineWave
{
    SineWave(SineWaveType waveType = SineWaveType::Directional, float waveLength = 0.1f, float amplitude = 0.05f,
             float waveSpeed = 0.1f, const glm::vec2 &waveDir = glm::vec2(1.0f),
...
struct WaveConfigs
{
    WaveConfigs()
    {
        ...
        color = glm::vec3(0.4f, 0.6f, 0.6f);
        surfaceAlpha = 0.5f;

        waves.reserve(MAX_WAVE_COUNT);
        waves.push_back(SineWave(SineWaveType::Directional, 0.05f, 0.02f, 0.1f, glm::vec2(1.0f)));
        waves.push_back(SineWave(SineWaveType::Directional, 0.1f, 0.015f, 0.05f, glm::vec2(0.2f, 0.5f)));
        waves.push_back(SineWave(SineWaveType::Directional, 0.15f, 0.04f, 0.15f, glm::vec2(0.1f, -0.8f)));
    ...
    void UI()
    {
        ...
        ImGui::DragFloat("Surface Opacity", &surfaceAlpha, 0.001f, 0.0f, 1.0f, "%.3f");
        ImGui::Separator();
        ImGui::Text("Waves");
