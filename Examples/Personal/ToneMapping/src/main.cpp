...
int main()
{
    ...
    int w = 0, h = 0;
    app->SetWindowTitle("Tone Mapping");
    app->displayUI = false;
    ...
    shader->AddSource(Tools::read_file_content("./shaders/PBR.vert"), GL_VERTEX_SHADER);
    shader->AddSource(Tools::read_file_content("./shaders/PBR.frag"), GL_FRAGMENT_SHADER);
    if (!shader->Compile())
        return -1;
