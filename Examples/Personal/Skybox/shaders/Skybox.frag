...
vec3 GetSkyboxColor(Surface surface)
{
    vec3 uvw = reflect(-surface.viewDir, surface.normDir);
    vec3 color = texture(map_Skybox, uvw).rgb;
    float fresnel = 1.0 - clamp(dot(surface.normDir, surface.viewDir), 0.0, 1.0);
    fresnel = fresnel * fresnel;
    fresnel = fresnel * fresnel;
    float fresnelSurface = clamp(1.0 - surface.roughness + 0.04, 0.0, 1.0);
    vec3 reflection = color * mix(surface.colorSpecular, vec3(fresnelSurface), fresnel);
...
void main()
{
    ...
    vec3 accColor = GetSkyboxColor(surface);
    if (val_HASPBR)
    {
        // go PBR path
