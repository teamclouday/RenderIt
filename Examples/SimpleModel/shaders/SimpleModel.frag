...
void main()
{
    ...
    vec3 colAmbient = val_AMBIENT * (map_AMBIENT_exists ?
        texture(map_AMBIENT, vertOut.texCoords).rgb :
        vec3(0.1));

    vec3 colDiffuse = val_DIFFUSE * (map_DIFFUSE_exists ?
    ...
    vec3 colEmissive = val_EMISSIVE * (map_EMISSIVE_exists ?
            texture(map_EMISSIVE, vertOut.texCoords).rgb :
            vec3(0.0));
