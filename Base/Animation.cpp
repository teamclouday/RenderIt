...
{
    const std::string LOGNAME = "Animation";
    if (!anim)
    {
        Tools::display_message(LOGNAME, "invalid animation data", Tools::MessageType::WARN);
        return;
    }
    ...
    if (duration <= 0.0f)
        Tools::display_message(LOGNAME, "invalid duration (" + std::to_string(duration) + ")",
    ...
    for (auto channelIdx = 0u; channelIdx < anim->mNumChannels; ++channelIdx)
    {
        ...
        if (bones.count(boneName))
            Tools::display_message(LOGNAME, "duplicate bone names (" + boneName + ")", Tools::MessageType::WARN);
